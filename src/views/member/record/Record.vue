    ...
    <div class="breastfeeding-record-page bg-white">
        ...
        <div class="page-content">
            <div class="h-full " style="overflow-y: auto;">
                <van-list v-model:loading="loading" :finished="finished" finished-text="没有更多了" @load="onLoad" class="h-full">
                    <div v-if="listData.length > 0" class="van-clearfix">
                        <div v-for="group in groupedAndSortedData" :key="group.date" class="record-item-container pb-5">
                            ...
                            <div v-for="item in group.items" :key="item.id" class="record-item m-3">
                                ...
                                <div class="ml-5">
                                    <div class="border-0 shadow-1 p-3 border-round-lg relative overflow-hidden" :class="{  'bg-orange-50 border-orange-100': item.details.喂奶方式 === '母乳瓶喂', 'bg-teal-50 border-teal-100': item.details.喂奶方式 === '配方奶粉', 'bg-purple-50 border-purple-100': item.details.喂奶方式 === '母乳亲喂'}">
                                        ...
                                        <div class="sticky " style="z-index: 1;">
                                            <div class="flex gap-3">
                                                <img v-if="item.details.喂奶方式 === '母乳亲喂'"  src="@@/icon/weinai.png" width="30px" height="30px" style="transform: rotate(0deg);"/>
                                                <img v-if="item.details.喂奶方式 !== '母乳亲喂'"  src="@@/icon/naiping.png" width="30px" height="30px" style="transform: rotate(45deg);"/>
                                                <div>
                                                    <div class="text-lg font-bold">{{ item.details.喂奶方式 }}</div>
                                                    <div class="flex gap-3 mt-2">
                                                        <div v-if="item.details.喂奶方式 === '母乳亲喂'" class="text-xs text-600 mt-2">
                                                        ...
                                                        <div v-if="item.details.喂奶方式 !== '母乳亲喂'" class="text-xs text-600 mt-2">
                                                            <div class="text-500 mb-1">瓶喂量</div>
                                                            <div class="text-900 text-lg">{{ item.details.瓶喂量 || 0 }}<span class="text-xs">ml</span></div>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
        ...
        <van-popup v-model:show="showFormPopup" position="bottom" :style="{ height: '100%' }" :close-on-popstate="true">
            <van-nav-bar title="添加喂奶记录" left-arrow @click-left="closeFormPopup" />
            <div class="popup-content">
                ...
                <div class="submit-btn-wrapper-popup">
                    <van-button type="primary" block round @click="submitRecord" :loading="isSubmitting">
                        提交记录
                    </van-button>
                </div>
            </div>
...
const onLoad = async () => {
    ...
    const { data } = await axios.get('member/get_record_lists', { p: currentPage, rule_id: 1, card_number: store.memberInfo.card_number }, { toast: 0 });
    if(data){
        console.log(11111, ...data.lists)
...
const submitRecord = async () => {
    ...
    try {
        ...
        const payload = {
            ...
            record_time: recordFormData.value['喂奶时间'],
...
onMounted(async () => {
    ...
    try {
        ...
        const { data } = await axios.get('member/get_record_rules', { id: 1 }, { toast: 0 });
        await new Promise(resolve => setTimeout(resolve, 500));
        closeToast();
        if (data.id) {
            rules.value = data;
